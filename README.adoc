...
This is a `Gradle` plugin that supports compiling, running and testing
`Scala` code on `JVM`, `Scala.js` and `Scala Native` backends.

Currently, plugin does NOT support Gradle projects that
mix code for multiple backends (JVM, Scala.js, Scala Native);
this is https://github.com/dubinsky/scalajs-gradle/issues/4[in the works].

Testing is supported for test frameworks that have a `sbt.testing.Framework`
implementation. Multiple test frameworks can be used in the same project at the same time
(supporting this probably is not a critical requirement,
but sbt does it, so I must too ;)).


For non-JVM backends, plugin does the linking.

If needed, plugin automatically:

- adds necessary backend-specific dependencies not added explicitly;
- adds necessary backend-specific `Scala` compiler parameters not added explicitly;
- for `Scala.js`, retrieves and installs `Node.js`;
- for `Scala.js`, installs configured Node modules using `npm`.
...
Which backend will be used depends on the value a project property
`{attribute-scalajsBackendProperty}` is set to:

- not set - `JVM`;
- set to `JVM` - `JVM`;
- set to `JS` - `Scala.js`;
- set to `Native` - `Scala Native`.

So, one way to use `Scala.js` backend is to
...
The Plugin automatically adds certain dependencies to various Gradle configurations
...
dependencies {
  ...
  testImplementation 'org.scalameta:munit_3:{version-framework-munit}'
  testImplementation 'org.scalacheck:scalacheck_3:{version-framework-scalacheck}'
  testImplementation 'org.scalatest:scalatest_3:{version-framework-scalatest}'
  testImplementation 'org.specs2:specs2-core_3:{version-framework-specs2}'
  testImplementation 'com.lihaoyi:utest_3:{version-framework-utest}'
  testImplementation 'dev.zio:zio-test-sbt_3:{version-framework-zio-test}'
}
----
...
dependencies {
  ...
  // Test framework(s) you use:
  testImplementation "org.scalameta:munit_$scala2versionMinor:{version-framework-munit}"
  testImplementation "org.scalacheck:scalacheck_$scala2versionMinor:{version-framework-scalacheck}"
  testImplementation "org.scalatest:scalatest_$scala2versionMinor:{version-framework-scalatest}"
  testImplementation "org.specs2:specs2-core_$scala2versionMinor:{version-framework-specs2-scala2}"
  testImplementation "com.lihaoyi:utest_$scala2versionMinor:{version-framework-utest}"
  testImplementation "dev.zio:zio-test-sbt_$scala2versionMinor:{version-framework-zio-test}"
}
----
...
dependencies {
  ...
  testImplementation 'org.scalameta:munit_sjs1_3:{version-framework-munit}'
  testImplementation 'org.scalacheck:scalacheck_sjs1_3:{version-framework-scalacheck}'
  testImplementation 'org.scalatest:scalatest_sjs1_3:{version-framework-scalatest}'
  testImplementation 'org.specs2:specs2-core_sjs1_3:{version-framework-specs2}'
  testImplementation 'com.lihaoyi:utest_sjs1_3:{version-framework-utest}'
}
----
...
dependencies {
  ...
  // Test framework(s) you use:
  testImplementation "org.scala-js:scalajs-junit-test-runtime_$scala2versionMinor:{version-framework-junit4-scalajs}"
  testImplementation "org.scalameta:munit_sjs1_$scala2versionMinor:{version-framework-munit}"
  testImplementation "org.scalacheck:scalacheck_sjs1_$scala2versionMinor:{version-framework-scalacheck}"
  testImplementation "org.scalatest:scalatest_sjs1_$scala2versionMinor:{version-framework-scalatest}"
  testImplementation "org.specs2:specs2-core_sjs1_$scala2versionMinor:{version-framework-specs2-scala2}"
  testImplementation "com.lihaoyi:utest_sjs1_$scala2versionMinor:{version-framework-utest}"
}
----
...
|JVM
|scalaCompilerPlugins
|only if JUnit4 for Scala.js is used
...
dependencies {
  ...
  scalanative "org.scala-native:tools_3:$scalaNativeVersion"
  scalanative "org.scala-native:test-runner_3:$scalaNativeVersion"

  scalaCompilerPlugins "org.scala-native:nscplugin_$scalaVersion:$scalaNativeVersion"
  scalaCompilerPlugins "org.scala-native:junit-plugin_$scalaVersion:$scalaNativeVersion"

  implementation "org.scala-native:scala3lib_native0.5_3:$scalaVersion+$scalaNativeVersion"
  implementation "org.scala-native:nativelib_native0.5_3:$scalaNativeVersion"
  implementation "org.scala-native:javaliblib_native0.5_3:$scalaNativeVersion"
  implementation "org.scala-native:clib_native0.5_3:$scalaNativeVersion"
  implementation "org.scala-native:posixlib_native0.5_3:$scalaNativeVersion"
  implementation "org.scala-native:windowslib_native0.5_3:$scalaNativeVersion"
  implementation "org.scala-native:auxlib_native0.5_3:$scalaNativeVersion"

  testImplementation "org.scala-native:test-interface_native0.5_3:$scalaNativeVersion"

  testImplementation 'org.scala-native:junit-runtime_native0.5_3:{version-framework-junit4-scalanative}'
  testImplementation 'org.scalameta:munit_native0.5_3:{version-framework-munit}'
  testImplementation 'org.scalacheck:scalacheck_native0.5_3:{version-framework-scalacheck}'
  testImplementation 'org.scalatest:scalatest_native0.5_3:{version-framework-scalatest}'
  testImplementation 'com.lihaoyi:utest_native0.5_3:{version-framework-utest}'
}
----
...
dependencies {
  ...
  scalanative "org.scala-native:tools_$scala2versionMinor:$scalaNativeVersion"
  scalanative "org.scala-native:test-runner_$scala2versionMinor:$scalaNativeVersion"

  scalaCompilerPlugins "org.scala-native:nscplugin_$scalaVersion:$scalaNativeVersion"
  scalaCompilerPlugins "org.scala-native:junit-plugin_$scalaVersion:$scalaNativeVersion"

  implementation "org.scala-native:scalalib_native0.5_3:$scalaVersion+$scalaNativeVersion"
  implementation "org.scala-native:nativelib_native0.5_$scala2versionMinor:$scalaNativeVersion"
  implementation "org.scala-native:javaliblib_native0.5_$scala2versionMinor:$scalaNativeVersion"
  implementation "org.scala-native:clib_native0.5_$scala2versionMinor:$scalaNativeVersion"
  implementation "org.scala-native:posixlib_native0.5_$scala2versionMinor:$scalaNativeVersion"
  implementation "org.scala-native:windowslib_native0.5_$scala2versionMinor:$scalaNativeVersion"
  implementation "org.scala-native:auxlib_native0.5_$scala2versionMinor:$scalaNativeVersion"

  testImplementation "org.scala-native:test-interface_native0.5_$scala2versionMinor:$scalaNativeVersion"

  testImplementation 'org.scala-native:junit-runtime_native0.5_$scala2versionMinor:{version-framework-junit4-scalanative}'
  testImplementation 'org.scalameta:munit_native0.5_$scala2versionMinor:{version-framework-munit}'
  testImplementation 'org.scalacheck:scalacheck_native0.5_$scala2versionMinor:{version-framework-scalacheck}'
  testImplementation 'org.scalatest:scalatest_native0.5_$scala2versionMinor:{version-framework-scalatest}'
  testImplementation 'com.lihaoyi:utest_native0.5_$scala2versionMinor:{version-framework-utest}'
}
----

== JVM

TODO document that the run task can be added by applying `id 'application'` Gradle plugin. Well, for mixed stuff it does not get the JVM classes...
Add my own?

TODO maybe adding overall dependencies will do it...
...
a JUnit Scala compiler plugin is also needed (<<junit4-scalajs-scala-2>>):
...
a JUnit Scala compiler plugin is also needed (<<junit4-scalanative>>):
...
TODO 'org.scala-native:test-interface_native0.5',
which the plugin adds, brings Scala Native JUnit4 in indirectly,
so the test will compile even without it being listed explicitly;
but since Scala compiler plugin that produces the bootstrappers
is only added by the plugin when JUnit4 is listed explicitly,
no tests are run!
Is there a smaller dependency or should I ask the Scala Native people to
drop this transitivity?
...
[#junit4-scalanative]
=== JUnit4 for Scala Native
Turns out, `JUnit4 for Scala Native` assumes existence of a `bootstrapper`
in every test class - apparently, because test discovery for `JUnit4`
is based on annotations, and reflection on Scala Native is not powerful enough,
so tests are pre-discovered _at compile time_,
and JUnit4-specific bootstrappers generated for them.

To get the bootsrappers generated on Scala 2,
a dedicated Scala compiler plugin has to be added: `org.scala-native:junit-plugin`.
This compiler plugin can _only_ be added when `JUnit4 for Scala Native`
is actually on the classpath - or Scala compiler breaks ;)
...
Scala Native follows the trail blazed by Scala.js, and I follow :)
...
and that is all it took to add support for Scala Native.

Some additional insights were found in
https://github.com/com-lihaoyi/mill/blob/main/libs/scalanativelib/worker/0.5/src/mill/scalanativelib/worker/ScalaNativeWorkerImpl.scala[Mill].
