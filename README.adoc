...
:icons: font
// INCLUDED ATTRIBUTES
:attribute-gradleVersionForBadge: 9.7.0
:attribute-pluginBackendProperty: org.podval.tools.backend
:version-gradle: 9.7.0
:version-plugin: 1.0.4
:version-scalanative: 0.5.12
:version-scalajs: 1.22.0
:version-framework-specs2-scala2: 4.23.0
:version-scala: 3.8.4
:version-scala-213: 2.13.18
:version-scala-212: 2.12.21
:version-sbt-test-interface: 1.0
:version-scalajs-dom: 2.8.1
:version-scalajs-env-jsdom-nodejs: 1.1.1
:version-node: 26.7.0
:version-junit: 4.13.2
:version-framework-junit4: 0.13.3
:version-framework-junit4-scala-js: 1.22.0
:version-framework-junit4-scala-native: 0.5.12
:version-framework-airspec: 2026.2.2
:version-framework-hedgehog: 0.13.1
:version-framework-munit: 1.3.5
:version-framework-scalacheck: 1.19.0
:version-framework-scalaprops: 0.11.0
:version-framework-scalatest: 3.2.20
:version-framework-specs2: 5.9.1
:version-framework-utest: 0.9.5
:version-framework-weaver-test: 0.13.0
:version-framework-ziotest: 2.1.26
// INCLUDED ATTRIBUTES
...
image:https://api.star-history.com/svg?repos=dubinsky/scalajs-gradle&type=Date[Star History Chart]
...
Note: Starting with Scala 3.8.0, the following has to be configured in the
Gradle build file:

[source,groovy]
----
tasks.withType(ScalaCompile).configureEach {scalaCompileOptions.additionalParameters = ['-release:17'] }
----
...
backend used is selected by the project property `{attribute-pluginBackendProperty}`.
...
[source,properties,subs="+attributes"]
----
{attribute-pluginBackendProperty}=js
----
...
- `esVersion` of at least `'2022'`

`useJSPI` enables Scala.js JSPI (`js.async` / `js.await`) and needs a JS engine that supports JSPI.
Environment variables `SCALAJS_USE_WEBASSEMBLY` and `SCALAJS_USE_JSPI` can override the defaults.
...
dependencies {
  if (scalaBackend.scala3) {
    implementation scalaBackend.scalaDependency('org.scala-native', 'scala3lib', "${scalaBackend.scalaVersion}+0.5.9", {it.scala3()}) // sets scalaBackend.backendVersion
  } else {
    implementation scalaBackend.scalaDependency('org.scala-native', 'scalalib', "${scalaBackend.scalaVersion}+0.5.9", {it.scala2()}) // sets scalaBackend.backendVersion
  }
...
|js
|{version-framework-junit4-scala-js}
|Scala 2

|JUni4 for Scala Native
|org.scala-native:junit-runtime
|native
|{version-framework-junit4-scala-native}
|
...
|latest that supports Scala 2: {version-framework-specs2-scala2}
...
|jvm, js, native
|{version-framework-weaver-test}
|

|ZIO Test
|dev.zio:zio-test-sbt
|jvm, js, native
|{version-framework-ziotest}
|
...
dependencies {
  ...
  testImplementation "org.typelevel:weaver-cats:{version-framework-weaver-test}"
  testImplementation "dev.zio:zio-test-sbt${scalaBackend.suffix}:{version-framework-ziotest}"
}
----
...
dependencies {
 ...
 if (!scalaBackend.scala3) {
  ...
  testImplementation scalaBackend.testFramework(WeaverTest, '{version-framework-weaver-test}')
  testImplementation scalaBackend.testFramework(ZioTest, '{version-framework-ziotest}')
}
----
...
(see link:src/main/scala/org/podval/tools/platform/Scala212Collections.scala[Scala212Collections]).
...
=== AsciiDoc
GitHub stupidly disables AsciiDoc includes in README;
...
- when tests are forked, `MaxNParallelTestDefinitionProcessor` is used, which already does that, so I do not need to;
...
Each `TestWorker` has an `idGenerator` which it uses to generate `WorkerTestDefinitionProcessor.workerSuiteId`;
...
I use a `RunTestDefinitionProcessor.rootTestSuiteIdPlaceholder`
...
to return `1` on `Scala.js` to prevent `MaxNParallelTestDefinitionProcessor`
from forking.
...
(see link:src/main/scala/org/podval/tools/test/run/DefaultTestExecuter.scala[DefaultTestExecuter]).
...
link:src/main/scala/org/podval/tools/test/detect/SbtTestFramework.scala[SbtTestFramework];
- to set test framework on the test task, I had to use reflection
in link:src/main/scala/org/podval/tools/build/TestTask.scala[TestTask];
...
* https://github.com/scalameta/munit/issues/1075[issues/1075]
Long options broken on js-native.
Thank you to https://github.com/kitbellew[kitbellew]
for clarification on the change of approach to the framework options.
...
Thank you to https://github.com/xerial[xerial] for approving.
