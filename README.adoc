...
:attribute-scalajsScalaBackendProperty: org.podval.tools.scalajs.backend
:attribute-scalajsIncludeSharedProperty: org.podval.tools.scalajs.includeShared
:attribute-scalajsScalaVersionProperty: org.podval.tools.scalajs.scalaVersion
// INCLUDED ATTRIBUTES
...
backend used is selected by the project property `{attribute-scalajsScalaBackendProperty}`.
...
----
{attribute-scalajsScalaBackendProperty}=js
----
...
(so there is no need to set property `{attribute-scalajsScalaBackendProperty}` manually in the backend-specific `gradle.properties`);
...
[source,groovy,subs="+attributes"]
----
import org.podval.tools.scalajsplugin.ScalaVersionData
final ScalaVersionData scalaVersion = ScalaVersionData.get(project)
...
import org.podval.tools.scalajsplugin.ScalaBackendData
final ScalaBackendData backend = ScalaBackendData.get(project)
