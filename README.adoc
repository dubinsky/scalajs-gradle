...
:version-framework-airspec: 2025.1.14
:version-framework-hedgehog: 0.12.0
...
:version-framework-utest: 0.9.0
:version-framework-weaver: 0.9.3
:version-framework-zio-test: 2.1.20
:attribute-pluginScalaBackendProperty: org.podval.tools.backend
:attribute-gradleVersionForBadge: 9.0.0
// INCLUDED ATTRIBUTES
...
There is no need (nor point) to have an `src` directory in the overall project:

- backend-specific sources reside in the backend-specific subprojects;
- sources shared between backends reside in the shared subprojects.
...
|currently not supported on Scala Native
...
_Less discarding of test failure throwables_
