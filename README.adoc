...
link:src/main/scala/org/podval/tools/scalajs/jvm/JvmTestTask.scala[org.podval.tools.scalajs.jvm.JvmTestTask].
Any such task will automatically depend on the `testClasses` task (and `testRuntimeClassPath`).

For Scala.js projects the type is
link:src/main/scala/org/podval/tools/scalajs/js/RunTask.scala[org.podval.tools.scalajs.js.Run.Test].
...
How is MUnit, which is JUnit4-based, supported by Scala.js - and JUnit4 itself is not?
