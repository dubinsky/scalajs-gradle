...
:attribute-pluginIncludeSharedProperty: org.podval.tools.scalajs.includeShared
:attribute-pluginScalaVersionProperty: org.podval.tools.scalajs.scalaVersion
// INCLUDED ATTRIBUTES
...
- some of the plugin's functionality depends on the scala version being set on the `scala` extension.
...
=== Scala Version Override
When property {attribute-pluginScalaVersionProperty} is set, e.g.:
[source,shell,subs="+attributes"]
----
$ ./gradlew -P{attribute-pluginScalaVersionProperty}={version-scala2}
----
...
(<<scala-version-specific-sources>>, <<scala-version-override>>, <<scala-backend-extension>>)
...
(so there is no need to set property `{attribute-pluginScalaBackendProperty}` manually in the backend-specific `gradle.properties`);
...
dependencies {
  ...
  implementation  "org.scala-js:scalajs-dom_sjs1_${scalaVersion.binary}:{version-scalajs-dom}"
  ...
  scalajs         "org.scala-js:scalajs-env-jsdom-nodejs_${scalaVersion.binary2}:{version-scalajs-env-jsdom-nodejs}"
  if (!scalaVersion.scala3) {
...
----
<1> if added manually, sets `scalaJsVersion` used for other automatically added dependencies
<2> only if JUnit4 for Scala.js is in the `testImplementation` configuration
...
----
<1> if added manually, sets `scalaNativeVersion` used for other automatically added dependencies
<2> only if JUnit4 for Scala Native is in the `testImplementation` configuration
...
dependencies {
  ...
  testImplementation "org.scalameta:munit${backend.suffix}_${scalaVersion.binary}:{version-framework-munit}"
  testImplementation "org.scalacheck:scalacheck${backend.suffix}_${scalaVersion.binary}:{version-framework-scalacheck}"
  testImplementation "org.scalatest:scalatest${backend.suffix}_${scalaVersion.binary}:{version-framework-utest}"
  testImplementation "com.lihaoyi:utest${backend.suffix}_${scalaVersion.binary}:{version-framework-utest}"

  if (backend.jvm) {
    testImplementation "dev.zio:zio-test-sbt${backend.suffix}_${scalaVersion.binary}:{version-framework-zio-test}"
  }

  if (backend.jvm || backend.js) {
    if (scalaVersion.scala3) {
      testImplementation "org.specs2:specs2-core${backend.suffix}_${scalaVersion.binary}:{version-framework-specs2}"
    } else {
      testImplementation "org.specs2:specs2-core${backend.suffix}_${scalaVersion.binary}:{version-framework-specs2-scala2}"
    }
  }
}
----
...
This approach was deemed too complicated to use
(and too fragile an implementation)
...
https://github.com/scala-native/scala-native/pull/4320[test failure throwable and duration] for Scala Native,
for adding `scala.scalanative.build.Build.buildCachedAwait()`
