...
=== Test Frameworks
Plugin replaces the `test` task with one that supports running sbt-compatible test frameworks; multiple test frameworks can be used at the same time.
...
JUnit5 is not supported since it uses its own test discovery mechanism
which the plugin is unlikely to ever support
(https://github.com/dubinsky/scalajs-gradle/issues/38[issues/38]).

TestNG is not supported since its
...
Note: Since ZIO Test tests are  objects with main method,
...
and that icon runs the whole class, not the test method it is for.
...
events for the tests in the nested suites have `NetsedTestSelector`.
