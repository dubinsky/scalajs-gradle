...
which the plugin does not support;
...
- plugin uses its own test detector, which does not support file-name based test scan,
  so `isScanForTestClasses` must be at its default value `true`;
...
- test filtering support is detailed in <<_test_filtering>>;
- test tagging support is detailed in <<_test_tagging>>.
...
so translation from the Gradle test case filtering to the SBT test interface filtering
can lose fidelity.
...
- `"org.podval.tools.test.*"`: tests in specified package (used by IntelliJ Idea, see <<_intellij_idea_integration>>);
- `"org.podval.tools.test.ScalaTest"`: tests in specified class (used by IntelliJ Idea, see <<_intellij_idea_integration>>).

All these patterns work as intended.

The following patterns specify test cases to run:

- `"org.podval.tools.test.JUnit4Test.success"`: specified test case in specified class (used by IntelliJ Idea, see <<_intellij_idea_integration>>);
...
- `JUnit4 for Scala.js`, `MUnit` on Scala.js and `uTest` do not support test selectors and
...
- `ScalaCheck` either does not run any test cases or runs all of them (TODO BUG?);
...
- `ZIO Test` in the presence of wildcard test case inclusions
  runs all tests in the class because of a https://github.com/zio/zio/pull/9680[bug].
...
TODO I get no events from ScalaTest nested test suit when running on JVM -
but I do when running on Scala.js!
