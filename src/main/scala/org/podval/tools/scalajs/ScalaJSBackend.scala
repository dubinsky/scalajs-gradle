...
import org.podval.tools.build.{Requirement, ScalaBinaryVersion, ScalaDependency, ScalaLibrary, Version}
...
object ScalaJSBackend extends NonJvmBackend(
  ...
  override protected def implementation(scalaLibrary: ScalaLibrary): Array[Requirement] =
    scalaLibrary.scalaVersion.binaryVersion match
      case _: ScalaBinaryVersion.Scala3 => Array(scala3Library(scalaLibrary.scalaVersion.version))
      case _ => Array.empty

  // There is no Scala 2 equivalent.
  private def scala3Library(version: Version): Requirement = scalaDependency(
