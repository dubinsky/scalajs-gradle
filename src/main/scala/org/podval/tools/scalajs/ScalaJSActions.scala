...
final class ScalaJSActions(
  nodePath: String, // TODO not needed for link()
  nodeEnvironment: Map[String, String], // TODO not needed for link()
  jsDirectory: File,
  reportBinFile: File,
  reportTextFile: File,
  optimization: Optimization, // TODO needed for link() only
  moduleKind: ModuleKind,
  moduleSplitStyle: ModuleSplitStyle, // TODO needed for link() only
  moduleInitializers: Option[Seq[ModuleInitializer]], // TODO needed for link() only
  prettyPrint: Boolean, // TODO needed for link() only
  runtimeClassPath: Seq[File], // TODO needed for link() only
  logger: Logger,
  logSource: String
):
  private def jsLogger: org.scalajs.logging.Logger = new org.scalajs.logging.Logger:
    ...
    override def log(level: JSLevel, message: => String): Unit =
      logger.log(ScalaJSActions.scalajs2gradleLevel(level), s"$logSource: $message")

  def link(): Unit =
    // Note: tests use fixed entry point
    val moduleInitializersSJS: Seq[ModuleInitializerSJS] = moduleInitializers
      .map(_.map(ScalaJSActions.toModuleInitializer))
      .getOrElse(Seq(ScalaJSActions.testAdapterInitializer))
    ...
    val linkerConfig: StandardConfig = StandardConfig()
      ...
      .withModuleKind(ScalaJSActions.toJs(moduleKind))
  ...
  private lazy val mainModule: Report.Module =
    ...
    require(ScalaJSActions.toJs(moduleKind) == result.moduleKind, s"moduleKind discrepancy: $moduleKind != ${result.moduleKind}")
    result
