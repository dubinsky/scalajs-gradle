...
trait JvmDependency extends Dependency:
  ...
  def fromVersion(scalaVersion: Option[Version], version: Version.Pre): WithVersion

  def withVersion(scalaLibrary: ScalaLibrary, version: Version): WithVersion
  ...
  final def require(version: Version = versionDefault): Requirement = Requirement(
    dependency = this,
    version = version
  )

  final def findInConfiguration(configuration: Configuration): Option[WithVersion] = configuration
    .getDependencies
    .asScala
    .toSet
  ...
  final def findInClasspath(classpath: Iterable[File]): Option[WithVersion] = classpath
    .map(Artifact.fromFile)
    .flatMap(find)
    .headOption

  private def find(artifact: Artifact): Option[WithVersion] = artifact
    .version
    .flatMap(Version.parse(isVersionCompound, _))
    .flatMap: (version: Version.Pre) =>
      val scalaVersion: Option[Version] = artifact.name.scalaVersion.map(Version(_))
      val extension: Option[String] = this.extension(version.version)
      val found: Boolean =
        isScalaVersion(scalaVersion) &&
        artifact.group.fold(true)(_ == group) &&
        (artifact.name.name == this.artifact) &&
        (artifact.name.backend == backend.artifactSuffix) &&
