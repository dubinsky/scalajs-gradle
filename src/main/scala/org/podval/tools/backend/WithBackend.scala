...
import org.podval.tools.build.{BackendTask, ClassPathAddition, DependencyRequirement, GradleClassPath, LinkTask, 
...
final class WithBackend(
  ...
  def apply(): Unit =
    ...
    project.afterEvaluate((_: Project) => afterEvaluate())
    ...
    backend.archiveAppendix.foreach(configureArchiveAppendix)
    backend.createExtension.foreach(_.apply(project))
    backend.pluginDependenciesConfigurationName.foreach(createPluginDependenciesConfiguration(_, backend.name))
    registerTasks()
  
  private def afterEvaluate(): Unit =
    ...
    dependencyRequirements(
      ...
      backend.pluginDependenciesConfigurationName
    ).foreach(_.apply(project))

    configureScalaCompile(scalaVersion)

    ClassPathAddition.Many(backend
  ...
  private def configureTestTasks(): Unit = project
    .getTasks
  ...
  private def configureLinkTasks(): Unit = project
    .getTasks
    .withType(classOf[LinkTask])
    .configureEach((task: LinkTask) =>
      val sourceSet: SourceSet = getSourceSet(task.isTest)
  ...
  private def registerTasks(): Unit =
    ...
    val link: Option[TaskProvider[?]] = registerTask(
      classOpt = backend.linkTaskClassOpt,
    ...
    // Register 'run' task.
    registerTask(
      classOpt = backend.runTaskClassOpt,
      name = "run",
      before = "Runs",
      after = "",
      group = "other",
    ...
    val linkTest: Option[TaskProvider[?]] = registerTask(
      classOpt = backend.testLinkTaskClassOpt,
    ...
    registerTask(
      classOpt = Some(backend.testTaskClass),
  ...
  private def dependencyRequirements(
    ...
    val requirements: ScalaBackend.DependencyRequirements = backend.dependencyRequirements(
    ...
    pluginDependenciesConfigurationName.toSeq.map(
      DependencyRequirement.Many(requirements.pluginDependencies      , pluginScalaVersion , _)
    ) ++ Seq(
      DependencyRequirement.Many(requirements.implementation          , projectScalaVersion, implementationConfigurationName          ),
      DependencyRequirement.Many(requirements.testRuntimeOnly         , projectScalaVersion, testRuntimeOnlyConfigurationName         ),
      DependencyRequirement.Many(requirements.scalaCompilerPlugins    , projectScalaVersion, scalaCompilerPluginsConfigurationName    ),
      DependencyRequirement.Many(requirements.testScalaCompilerPlugins, projectScalaVersion, testScalaCompilerPluginsConfigurationName)
    )
