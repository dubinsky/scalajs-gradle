...
object ScalaJSBuild:
  ...
  def run(
     ...
     * server mode.
     */
      val jsRun: JSRun = mkJsEnv(node, adjustJSEnvKind(jsEnvKind, moduleKindEffective), useWebAssembly, logSource).start(
    ...
    ScalaJSTestEnvironment(
      ...
      testAdapter = new NonJvmTestAdapter:
        ...
        private lazy val testAdapter: TestAdapter = TestAdapter(
          jsEnv = mkJsEnv(node, adjustJSEnvKind(jsEnvKind, moduleKindEffective), useWebAssembly, logSource),
  ...
  private def adjustJSEnvKind(
    ...
    if jsEnvKind == JSEnvKind.JSDOMNodeJSEnv && moduleKind != ModuleKind.NoModule then
