...
//    result
  ...
  def writeStrings(array: Array[String], separator: String): String =
    var result: String = ""
    var i: Int = 0
  ...
  def readStrings(string: String, separator: String): Array[String] =
    // Now that we split the containing string with "-1",
    // empty string turns into an array with one empty string in it...
    val strings: Array[String] = string.split(separator)
