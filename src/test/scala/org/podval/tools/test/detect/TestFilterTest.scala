...
class TestFilterTest extends AnyFlatSpec, TableDrivenPropertyChecks, Matchers:
  ...
  it should "calculate included methods" in:
    val data: TableFor3[String, String, Option[TestFilterMatch]] = Table(
      ...
      ("some.package.*", "some.package.Foo", Some(Suite(true))),
      ("some.package*", "some.package.Foo", Some(Suite(true))),
      ("some.package*", "some.package.sub.Foo", Some(Suite(true))),
      ("some.package*", "some.package1.Foo", Some(Suite(true))),
      ("some.package*", "some.other.package.Foo", None),

      // method in a package
      ("some.package*.someMethod", "some.package.Foo", Some(Tests(Set("someMethod"), Set.empty))),
      ("some.package*.some*", "some.package.Foo", Some(Tests(Set.empty, Set("some")))),

      // all tests in some class
      ("SomeClass", "SomeClass", Some(Suite(true))),
      ("some.package.SomeClass", "some.package.SomeClass", Some(Suite(true))),

      // all tests in some class in any package
      ("SomeClass", "any.package.SomeClass", Some(Suite(true))),
      ("*.SomeClass", "any.package.SomeClass", Some(Suite(true))),

      // single specified test in some class
      ("SomeClass.someMethod", "SomeClass", Some(Tests(Set("someMethod"), Set.empty))),
      ("some.package.SomeClass.someMethod", "some.package.SomeClass", Some(Tests(Set("someMethod"), Set.empty))),
      ...
      ("SomeClass.someMethod", "any.package.SomeClass", Some(Tests(Set("someMethod"), Set.empty))),
      // TODO     ("*.SomeClass.someMethod", "any.package.SomeClass", Some(Tests(Set("someMethod"), Set.empty))),

      // specified tests in some class
      ("SomeClass.*someMethod*", "SomeClass", Some(Tests(Set.empty, Set("someMethod")))),

      // method name containing spaces
      ("org.gradle.SomeTestClass.some method containing spaces", "org.gradle.SomeTestClass", Some(Tests(Set("some method containing spaces"), Set.empty))),

      // include all integration tests
      ("*IntegTest", "XIntegTest", Some(Suite(true))),
